...
func evaluar(m1, m2 float64) {
	...
	for {
		...
		predictedVal := predecir(score, m1, m2)
	...
	precision := float64(truePosNeg) / float64(len(observed))

	fmt.Printf("\nPrecisión = %0.2f\n\n", precision)
}

func predecir(score, m1, m2 float64) float64 {
